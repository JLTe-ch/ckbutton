CKEDITOR.dialog.add('ckbutton', function (editor) {
  var lang = editor.lang.ckbutton;
  var styles = [];
  var buttonStyles = editor.config.buttonStylesSet;
  for (var i = 0; i < buttonStyles.length; i++) {
    styles.push([buttonStyles[i].name, i])
  }
            ...
            children: [
              {
                id: "styles",
                type: "select",
                label: lang.buttonStylesLabel,
                items: styles,
                setup: function (widget) {
                  this.setValue(widget.data.styles || '');
                },
                commit: function (widget) {
                  widget.setData('styles', this.getValue());
                }
              }
            ]
          },
